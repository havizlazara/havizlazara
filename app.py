...
def load_data():
    # Menambahkan ttl=0 untuk memastikan data selalu fresh dari GSheets
...
# --- FUNGSI UPDATE CLOUD (REVISI UTAMA) ---
def save_to_gsheets(df_to_save):
    try:
        # Menghapus kolom tambahan 'Pilih' sebelum simpan jika ada
        df_clean = df_to_save.drop(columns=['Pilih'], errors='ignore')
        conn.update(data=df_clean)
        st.cache_data.clear() # Paksa hapus cache
        return True
...
if st.session_state['authenticated']:
    ...
    with tab_monitor:
        ...
        if not df_f.empty:
            ...
            with g1: st.plotly_chart(px.bar(df_f['PIC'].value_counts().reset_index(), x='index', y='PIC', color='index', height=350, title="By PIC"), use_container_width=True)
            with g2: st.plotly_chart(px.pie(df_f, names='Status', hole=.4, height=350, title="By Status"), use_container_width=True)
            with g3: st.plotly_chart(px.bar(df_f['Unit no'].value_counts().nlargest(5).reset_index(), x='index', y='Unit no', height=350, title="Top 5 Units"), use_container_width=True)

        st.markdown("---")
        df_ed = df_f.copy()
        if 'Pilih' not in df_ed.columns: df_ed.insert(0, 'Pilih', False)
        edited_table = st.data_editor(df_ed, use_container_width=True, hide_index=True, height=400, key="main_editor", column_config={"Delivery Date": st.column_config.TextColumn("Delivery Date"), "Doc Date": st.column_config.TextColumn("Doc Date"), "Last Update": st.column_config.TextColumn("Last Update", disabled=True)})
        
        if st.button("💾 SAVE ALL TO GSHEET", type="primary"):
            if save_to_gsheets(st.session_state.df_master):
                st.balloons()
                st.success("🎉 DATA BERHASIL DISIMPAN KE GOOGLE SHEETS!")
                if st.button("SAYA MENGERTI"): st.rerun()

    with tab_personal:
        st.markdown("### 👤 Personal Monitoring & Revision")
        df_p_master = st.session_state.df_master.copy()
        cp1, cp2 = st.columns(2)
        pic_opts = sorted([str(x) for x in df_p_master['PIC'].unique() if x])
        f_pic = cp1.selectbox("Filter PIC Name:", options=["All"] + pic_opts)
        po_opts = sorted([str(x) for x in df_p_master['PO No'].unique() if x])
        ...
        if st.button("🚀 CONFIRM REVISION & SAVE TO GSHEET", type="primary"):
            ...
            # REVISI: Langsung simpan ke cloud
            if save_to_gsheets(st.session_state.df_master):
                st.toast("🚀 Revisi Berhasil Terkirim ke Cloud!", icon="✅")
                st.info("Pesan: Revisi dan Google Sheets sudah tersinkronisasi.")
                if st.button("KONFIRMASI: DATA SUDAH UPDATE"): st.rerun()
    ...
    with tab_bulk:
        ...
        def run_bulk_final(status_val, note_val):
            today_str = datetime.now().strftime("%d-%m-%Y")
            updated = 0
            ...
            if updated > 0 and save_to_gsheets(st.session_state.df_master):
                ...
                st.toast(f"✅ {updated} Data Update & Save Cloud Sukses!")
                st.rerun()

        c1, c2, c3 = st.columns(3)
        if c1.button("🔴 Set Outstanding"): run_bulk_final("Outstanding", "")
        if c2.button("🟢 Set Bitung Complete"): run_bulk_final("Complete", "Receive at Bitung")
        if c3.button("🟢 Set Site Complete"): run_bulk_final("Complete", "Receive at Site")
    ...
    with tab_daily:
        ...
        if st.button("🚀 INSERT & AUTO SAVE"):
            clean_new = st.session_state.daily_df[st.session_state.daily_df['PO No'].str.strip() != ""].copy()
            if not clean_new.empty:
                ...
                if save_to_gsheets(st.session_state.df_master):
                    st.toast("✅ Data Baru Berhasil Masuk Cloud!")
                    st.session_state.daily_df = pd.DataFrame(columns=COLUMNS_ORDER)
