...
# --- 3. CSS CUSTOM (FOKUS UKURAN FONT) ---
st.markdown("""
    <style>
    /* 1. Judul Tab (Tab Titles) - 2x Lebih Besar */
    button[data-baseweb="tab"] div p {
        font-size: 28px !important;
        font-weight: bold !important;
    }
    
    /* 2. Label Filter (Input Labels) - 2x Lebih Besar */
    ...
    /* 3. Judul Kolom Tabel (Table Header Cells) - 2x Lebih Besar */
    ...
    .custom-header {
        ...
        background-color: #1f4e79; border: 3px solid #1f4e79;
    }
    .giant-title { font-size: 45px; font-weight: 900; color: white !important; }
    </style>
...
with tab_monitor:
    ...
    f_dept = c1.multiselect("Filter Dept", options=sorted(st.session_state.df_master['Dept.'].unique()))
    if f_dept: df_f = df_f[df_f['Dept.'].isin(f_dept)]
    f_fleet = c2.multiselect("Filter Fleet", options=sorted(df_f['Fleet'].unique()))
    if f_fleet: df_f = df_f[df_f['Fleet'].isin(f_fleet)]
    f_unit = c3.multiselect("Filter Unit", options=sorted(df_f['Unit no'].unique()))
    if f_unit: df_f = df_f[df_f['Unit no'].isin(f_unit)]
    f_stat = c4.multiselect("Filter Status", options=sorted(df_f['Status'].unique()))
    if f_stat: df_f = df_f[df_f['Status'].isin(f_stat)]

    search_q = st.text_input("Search PO / Supplier:", placeholder="Ketik di sini...")
    if search_q:
        df_f = df_f[df_f.apply(lambda r: r.astype(str).str.contains(search_q, case=False).any(), axis=1)]

    st.markdown("---")
    st.markdown("### 📋 Database Monitoring")
    calc_h = min(max((len(df_f) + 1) * 35 + 60, 250), 800)
    ...
    if st.session_state['authenticated']:
        ...
        def apply_style(row):
            ...
            return [c_f] * len(row) if row['Pilih'] else [''] * len(row)
